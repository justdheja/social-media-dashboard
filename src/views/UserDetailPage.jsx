						...
						{albums.map((album) => (
							<div className="my-2">
								...
								<Link
									...
									className="bg-green-500 p-2 rounded text-xs text-white font-bold"
								>
									See Photos
								</Link>
							</div>
				...
				<div className="pl-4">
					...
					{posts.map((post) => (
						<div className="text-justify my-2" key={post.id}>
							...
							<Link
								to={`/users/${profile.id}/posts/${post.id}`}
								className="bg-green-600 p-2 my-1 rounded text-xs font-bold"
							>
								Detail
							</Link>
						</div>
